...
# extract 3rd position
def extract_3(dis_code):
    return dis_code.split("(", 1)[0].split(".")[2]


# collect info on semesters for a discipline
def total_sem(data_frame, subfield_name, subj, component, subj_code):
    ...
    sem_list = list(dict.fromkeys(df_1["SEMESTER"].to_list()))
    cred_list = list(dict.fromkeys(df_1["CREDITS"].to_list()))
    return "".join(map(str, sem_list)) + "/" + "".join(str(c) for c in cred_list).replace(".0", "")


# calculate 4th position
...
def generate_df_w_unique_code(in_df, sys_df=None):
    code_list = []
    out_df = in_df.copy()
    if (sys_df is None) or sys_df.empty or ("DIS_CODE" not in sys_df.columns):
        cols = list(in_df)
        cols.append("DIS_CODE")
        sys_df = pd.DataFrame(columns=cols)
    sys_df["SEM_INFO"] = [total_sem(sys_df,
                                    sys_df["SUBFIELDNAME"].values[r],
                                    sys_df["SUBJECT"].values[r],
                                    sys_df["COMPONENT"].values[r],
                                    sys_df["SUBJECT_CODE"].values[r])
                          for r in range(0, len(sys_df["SUBJECT"]))
                          ]
